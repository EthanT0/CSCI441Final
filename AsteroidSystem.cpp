...
AsteroidSystem::AsteroidSystem() {
    ...
    for (int i = 0; i < 100; i ++) { // Make 100 asteroids
        Asteroid asteroid = Asteroid("textures/asteroid.jpg", // Texture path
                                     glm::vec3(randNumber(20), randNumber(20), randNumber(20)), // Initial position
                                     glm::vec3(1, 1, 1),  // Axis of rotation
                                     glm::vec3(1, 1, 1)); // Scale

        asteroids.push_back(asteroid);
        translations.push_back(glm::vec3(randNumber(2), randNumber(2), randNumber(2))); // Generate random initial translation vector
    }

}

void AsteroidSystem::draw(glm::mat4 viewMatrix, glm::mat4projMatrix) {
    float angle = 0;
